...
export function JobsView() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="glass-card p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-sm font-semibold text-ink"><BriefcaseBusiness className="size-4 text-primary" />Choose career boards</div><p className="mt-1 max-w-xl text-xs leading-5 text-muted">Add company board IDs separated by commas. These are the last part of a public careers URL, such as <span className="font-medium text-ink">google</span> or <span className="font-medium text-ink">stripe</span>.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:min-w-[500px]"><label className="text-xs font-medium text-muted">Greenhouse company IDs<Input value={greenhouseBoards} onChange={(event) => setGreenhouseBoards(event.target.value)} placeholder="airbnb, google" className="mt-1.5 h-9" /></label><label className="text-xs font-medium text-muted">Lever company IDs<Input value={leverBoards} onChange={(event) => setLeverBoards(event.target.value)} placeholder="netflix, stripe" className="mt-1.5 h-9" /></label></div><Button className="h-9 shrink-0" onClick={loadJobs} disabled={loading}><RefreshCw className={`size-3.5 ${loading ? "animate-spin" : ""}`} />Update results</Button></div></Card>
    <section className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><SlidersHorizontal className="mr-1 size-4 text-muted" /><span className="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Filter by source</span>{sourceOptions.map((item) => <button key={item} onClick={() => setSource(item)} className={`rounded-full px-3.5 py-2 text-xs font-semibold transition ${source === item ? "bg-primary text-white shadow-[0_8px_20px_rgba(91,53,230,.22)]" : "bg-white text-muted hover:bg-primary-soft hover:text-primary"}`}>{item}</button>)}</div><p className="mt-2 text-xs text-muted">Showing {filteredJobs.length.toLocaleString()} of {(data?.jobs.length ?? 0).toLocaleString()} openings</p></div><div className="flex flex-wrap gap-2"><label className="relative"><span className="sr-only">Workplace</span><select aria-label="Filter by workplace" value={workplaceFilter} onChange={(event) => setWorkplaceFilter(event.target.value as typeof workplaceFilter)} className="h-9 appearance-none rounded-xl border border-border bg-white pl-3 pr-8 text-xs font-medium text-muted outline-none focus:border-primary/50"><option>Any workplace</option>{workplaceOptions.slice(1).map((item) => <option key={item}>{item}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2.5 top-3 size-3 text-muted" /></label><label className="relative"><span className="sr-only">Sort jobs</span><select aria-label="Sort jobs" value={sort} onChange={(event) => setSort(event.target.value)} className="h-9 appearance-none rounded-xl border border-border bg-white pl-3 pr-8 text-xs font-medium text-muted outline-none focus:border-primary/50"><option>Newest first</option><option>Company A–Z</option></select><ChevronDown className="pointer-events-none absolute right-2.5 top-3 size-3 text-muted" /></label><Button variant="secondary" className="h-9 px-3 text-xs" onClick={loadJobs} disabled={loading}><RefreshCw className={`size-3.5 ${loading ? "animate-spin" : ""}`} />Refresh</Button></div></section>
